...
class XmlRpcScanner:
    ...
    def run(self):
        ...
        if self.metadata:
            node_properties["apiSchema"] = self.metadata

        node = self.jsonlib.create_node_object("xmlrpc_api")
        self.node_key = node.get("key")
        self.jsonlib.add_node(node)
        self.jsonlib.add_properties(properties=node_properties, node_key=self.node_key)
